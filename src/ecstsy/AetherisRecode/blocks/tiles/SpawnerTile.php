...
namespace ecstsy\AetherisRecode\blocks\tiles;

use ecstsy\AetherisRecode\blocks\CustomSpawner;
use ecstsy\AetherisRecode\entity\EntityManager;
...
class SpawnerTile extends Spawnable
{
    ...
    {
        $this->spawnDelay = $nbt->getShort(self::TAG_SPAWN_DELAY, 0);
        ...
        $this->legacyEntityTypeId = $nbt->getShort(self::TAG_LEGACY_ENTITY_TYPE_ID, 0);
    ...
    private function findNearestEntity(string $type): ?SpawnerEntity
    {
        ...
        foreach (
            $this->getBlock()->getPosition()->getWorld()->getNearbyEntities(new AxisAlignedBB(
                $pos->x - 25,
                $pos->y - 25,
                $pos->z - 25,
                $pos->x + 25,
    ...
    public function setEntityId(string $id): void
    {
        ...
        if (($block = $this->getBlock()) instanceof CustomSpawner) {
